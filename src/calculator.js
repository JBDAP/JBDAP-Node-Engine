 ...
 */
function checkCondition(obj,relation,parent,root,self,lang) {
 ...
 * @param {string} lang 提示信息所用语言
 */
function compare(comparision,parent,root,self,lang) {
    if (JS._.isUndefined(lang)) lang = JE.i18nLang
    try {
        let left = comparision.left, right = comparision.right, operator = comparision.operator
        if (JS._.isString(left)) left = tag2value(left,parent,root,self)
        if (JS._.isString(right)) {
        ...
        switch (operator) {
            ...
            case 'notExist':
                return (left == '{NotExist}') == right
            case 'isNull':
                return JS._.isNull(left) == right
            case 'isNotNull':
                return JS._.isNull(left) != right
            case 'isUndefined':
                return JS._.isUndefined(left) == right
            case 'isNotUndefined':
                return JS._.isUndefined(left) != right
            case 'isEmpty':
                return (left == '') == right
 ...
 * @param {string} lang 提示信息所用语言
 */
function tag2value(tag,parent,root,self,lang) {
    if (JS._.isUndefined(lang)) lang = JE.i18nLang
    try {
        // 要取值的对象
        let target = null
        ...
        if (slices.length === 1) {
            ...
            return JS._.uniq(JS._.map(val,slices[1]))
        }
        ...
        switch (obj.operator) {
            case 'count': {
                ...
                JS._.forEach(list, (item) => {
                    // 有该属性且不为 null 才为有效计数
                    if (Object.keys(item).indexOf(obj.fields) >= 0 && !JS._.isNull(item[obj.fields])) result++ 
                })
                return result
            }
            case 'sum': {
                ...
                return JS._.sumBy(list,obj.fields)
            }
            case 'avg': {
                if (list.length === 0) return null
                let slices = obj.fields.split(',')
                ...
                return JS._.meanBy(list,obj.fields)
            }
            case 'max': {
                if (list.length === 0) return null
                let slices = obj.fields.split(',')
                ...
                return JS._.maxBy(list,obj.fields)[obj.fields]
            }
            case 'min': {
                if (list.length === 0) return null
                let slices = obj.fields.split(',')
                ...
                return JS._.minBy(list,obj.fields)[obj.fields]
            }
            case 'first': {
                if (list.length === 0) return null
                let slices = obj.fields.split(',')
            ...
            case 'pick': {
                ...
                return JS._.uniq(JS._.map(list,obj.fields))
            }
            case 'clone': {
                if (list.length === 0) return null
                let fields = []
                let slices = obj.fields.split(',')
                JS._.forEach(slices, (item) => {
                    if (item !== '') fields.push(item)
                })
                fields = JS._.uniq(fields)
                ...
                JS._.forEach(list, (item) => {
                    ...
                    if (fields === '*') {
                        ...
                        JS._.forEach(keys, (key) => {
                            temp[key] = JS._.cloneDeep(item[key])
                        })
                    }
                    else {
                        JS._.forEach(fields, (field) => {
                            temp[field] = JS._.cloneDeep(item[field])
                        })
                    }
                    result.push(temp)
                })
